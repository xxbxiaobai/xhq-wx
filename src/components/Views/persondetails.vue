...
export default {
  ...
  data() {
    return {
      ...
      avatar: require('../../assets/Mrtx2.png'),
      RecordID: '',
      headerimage: null,
      identity: ''
    }
  ...
  methods: {
    ...
    Listdetails(idCardNo) {
      this.$axios({
        ...
        method: 'post',
        url: domain.testUrl + 'api-movement/movement/api/school/card/detail',
        data: Qs.stringify({ idCardNo: idCardNo })
      })
        .then(response => {
          ...
          if (response.data.code == 0) {
            ...
            this.avatar =
              domain.testUrl +
              'api-file/files-anon/download?fileID=' +
              str.headFileId
            // 判断头像数据
    ...
    FormValidation() {
      ...
      //   this.$set(val, 'upload', data)
      //   val = {upload: this.upload};
      // eslint-disable-next-line no-undef
      console.log(formdata.get('cmyId'))
      console.log(formdata.get('idCardNo'))
      this.SubmitToSave(formdata)
    },
    // 提交保存
    SubmitToSave(va) {
      ...
      this.$axios({
        ...
        method: 'post',
        url: domain.testUrl + 'api-movement/movement/api/school/card/update',
        data: va
      })
        .then(response => {
          console.log(response.data)
    ...
    BelongingTheCommunity() {
      this.$axios
        .get(
          domain.testUrl +
            'api-movement/movement/api/school/card/area?parentId=4',
          {
            headers: {
              'front-token': localStorage.getItem('apitoken')
            }
        ...
        .then(response => {
          ...
          if (response.data.code == 0) {
            ...
            // this.list = response.data;
            this.TheStreet(response.data.data[0].id)
            response.data.data.forEach(item => {
              //   this.communityA.push(item);
              //   this.columns2[0].values = item.name;
              this.columns2[0].values = Object.values(response.data.data).map(
                function(e) {
    ...
    TheStreet(id) {
      this.$axios
        .get(
          domain.testUrl +
            'api-movement/movement/api/school/card/area?parentId=' +
            id,
          {
            headers: {
              'front-token': localStorage.getItem('apitoken')
        ...
        .then(response => {
          ...
          if (response.data.code == 0) {
            ...
            response.data.data.forEach(item => {
              //   this.communityA.push(item);
              //   this.columns2[0].values = item.name;
              this.columns2[1].values = Object.values(response.data.data).map(
                function(e) {
                  return { text: e.name, id: e.id }
                }
              )
    ...
    afterRead(file) {
      ...
      this.$axios({
        method: 'post',
        url: domain.testUrl + 'api-file/files-anon/base',
        data: Qs.stringify({
          base: file.content
        })
      })
        .then(response => {
          if (response.data.resp_code === 0) {
            ...
            this.avatar =
              domain.testUrl +
              'api-file/files-anon/download?fileID=' +
              response.data.datas.id
            this.identity = response.data.datas.id
            this.isHead = response.data.datas.id
            // 取消等待
            Toast.clear()
          } else {
            Toast(response.data.resp_msg)
            // 取消等待
            Toast.clear()
          }
        })
