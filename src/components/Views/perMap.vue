<template>
  <div>
    <van-loading type="spinner" vertical v-show="showLoading"> </van-loading>
    <div class="tab">
      <div
        ...
        <div class="select">
          ...
          >
            <template slot="content">
              <div
                class="mainItem"
                v-for="(item, index) in street[id]"
                :key="index"
                @click="onClickItem(item.schoolName)"
              >
...
import { fitnessSiteInfo, fitnessMap } from '../../http/api'
var map, //地图
  myMapIndex, //我的位置
  dataPOI, //信息点
  count = null
export default {
  ...
  methods: {
    ...
    init(lat, lng) {
      ...
      // 地图图块加载完成后触发
      map.on('complete', () => {
        myMapIndex = [dataPOI.pointY, dataPOI.pointX] = [lng, lat]
    ...
    pointOfInformation(index) {
      ...
      .get(
        domain.testUrl +
            'api-movement/movement/api/siteDetails/fitnessMap',
          {
            headers: {
          ...
          if (res.code == '0') {
            ...
            let msg = res.data.map((item) => {
              return {
                position: [
                  item.schoolCoords.split(',')[0],
                  item.schoolCoords.split(',')[1],
                ],
    ...
    icon(page) {
      ...
      return page.map((item) => {
        ...
        icon.onclick = (e) =>
          this.iconCont(item.position, item.schoolName, item.site)
